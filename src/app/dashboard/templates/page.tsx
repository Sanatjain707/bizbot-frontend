...
export default function TemplatesPage() {
  ...
  const [form, setForm] = useState({ name: '', category: 'MARKETING', language: 'en', header: '', body: '', footer: '' })
  ...
  async function remove(t: any) {
    ...
    if (error) {
      ...
      showToast('Template deleted from Meta and BizBot', 'success')
    }
    setConfirmDel(null)
  }
  ...
  function TemplateCard({ t }: any) {
    ...
    return (
      <Card className="p-4">
        ...
        {t.header && <p className="text-xs font-semibold text-[#9AA0AB] mb-1">{t.header}</p>}
        <p className="text-sm text-[#9AA0AB] leading-relaxed whitespace-pre-wrap">{t.body}</p>
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-[rgba(255,255,255,0.06)]">
          <span className="text-xs text-[#5A6370]">{t.category} · {t.language}</span>
    ...
    <div className="animate-up">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button icon={Plus} onClick={() => setModal(true)}>New Template</Button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <Card className="py-4"><EmptyState icon={FileText} title="No templates yet" desc="Create your first template — it'll be sent to Meta for approval (~1 hour to 1 day)" action={<Button size="sm" icon={Plus} onClick={() => setModal(true)}>New Template</Button>} /></Card>
      ) : (
        <>
          {approved.length > 0 && (
            <div className="mb-6">
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="New Message Template" size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <Select label="Category" value={form.category} onChange={(e: any) => setForm(p => ({ ...p, category: e.target.value }))} options={[{value:'MARKETING',label:'Marketing (offers, promos)'},{value:'UTILITY',label:'Utility (updates, alerts)'}]} />
          </div>
          ...
          <Textarea label="Message Body *" rows={4} placeholder="Namaste {{1}}! Is Diwali, paayein 20% off sabhi services par. Book karein aaj hi! 🪔" value={form.body} onChange={(e: any) => setForm(p => ({ ...p, body: e.target.value }))} hint="Use {{1}} for the customer's name" />
        ...
        <p className="text-xs text-[#5A6370] mt-3">Meta reviews each template (usually within an hour). You'll see the status update here automatically.</p>
      </Modal>
